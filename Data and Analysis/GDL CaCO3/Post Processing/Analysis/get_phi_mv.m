...
for j = 1:size(to_show,1)
    ...
    for i = 1:iterations
        ...
        consts0 = consts_best(j,:) + (1-2*rand(1,num_consts)).*consts_best(j,:);
%         consts0 = consts_best(j,:) + (rand(1,num_consts))*5;
%         consts0 = consts_best(j,:) + (1-2*rand(1,4))*1;
        ...
        try 
            ...
            ODE_forward_func_MV(L0, consts_approx); % Plot the result

            if err < err_best(j,1)
                err_best(j,1) = err
                consts_best(j,1:num_consts) = consts_approx
...
%     model_H_ = model_H_(:,1);
    ...
    % Mixing higher conc
    [t_, x_] = ode15s(@(t_,x_) GDL_ODE_MODELMV(t_, x_, x0_, L0, kstar, consts), exp_t_, x0_);
...
%     err = norm(exp_H_(1:short_index) - H_(1:short_index))...
    ...
    err = norm(exp_H_ - H_)...
        + 40*norm(exp_H_(1:weight_end_index) - H_(1:weight_end_index))
